...
CLR_M=$'\e[35m'   # magenta (resultados)
CLR_B=$'\e[1m'    # bold
CLR_RESET=$'\e[0m'
...
usage(){
  ...
  printf "\t- CSV:\t\tdata/implX/sec.csv con mode=encrypt\n"
  exit "${code}"
}
...
encrypt_text(){
  ...
  out="$(uniq_bin)"
  if [[ -z "${t}" ]]; then
    printf "%b\t%s\t%s\n" "${CLR_D}" "skipping" "(texto vacío)"
    return 0
  fi

  printf "%b\t- %s%b\n" "${CLR_W}${CLR_B}" "encrypt(seq)" "${CLR_RESET}"
  printf "\t- %bkey:%b\t%b%s%b\n" "${CLR_B}" "${CLR_RESET}" "${CLR_W}" "${KEY_TRUE}" "${CLR_RESET}"

  printf "%bComando:%b\t" "${CLR_C}" "${CLR_RESET}"
  printf "\n\t- %b%q%b\n" "${CLR_Y}" "${BIN} encrypt ${KEY_TRUE} ${out} ${CSV} ${HOST}" "${CLR_RESET}"
  
  printf "\n%bResultados:%b\t" "${CLR_C}" "${CLR_RESET}"
  printf "\n\t- %borigen:%b\t%s\n" "${CLR_B}" "${CLR_RESET}" "${SRC_LABEL}"
  printf "\t- %boutput-file:%b\t%s\n" "${CLR_B}" "${CLR_RESET}" "${out}"

  # ejecutar y capturar salida para mostrar agrupada
  local tmpOut tmpErr rc
  tmpOut=$(mktemp) || exit 1
  tmpErr=$(mktemp) || { rm -f "${tmpOut}"; exit 1; }
  rc=0
  printf '%s' "${t}" | "${BIN}" encrypt "${KEY_TRUE}" "${out}" "${CSV}" "${HOST}" >"${tmpOut}" 2>"${tmpErr}" || rc=$?
  
  # mostrar stdout/stderr separados y con color
  if [[ -s "${tmpOut}" ]]; then
    printf "\t%bstdout:%b\n" "${CLR_W}" "${CLR_RESET}"
...
# ===== banner =====
printf "%b%s%b\n" "${CLR_M}${CLR_B}" "CIFRADO SECUENCIAL (DES) — impl=${IMPL} host=${HOST}" "${CLR_RESET}"

printf "\n%bConfiguración:%b\n" "${CLR_C}${CLR_C}" "${CLR_RESET}"
printf "\t- %bbinario:%b\t%s\n" "${CLR_B}" "${CLR_RESET}" "${BIN}"
